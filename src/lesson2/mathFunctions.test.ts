...
describe('Math functions test cases', () => {
  ...
  const resultOfSquare = Math.pow(value1, 2)
  it(`Squared ${value1} = ${resultOfSquare}`, () => {
    expect(square(value1)).toBe(resultOfSquare)
  })

  const resultOfMultiplication = value1 * value2
  it(`${value1} * ${value2} = ${resultOfMultiplication}`, () => {
    expect(multiplication(value1, value2)).toBe(resultOfMultiplication)
  })
})
